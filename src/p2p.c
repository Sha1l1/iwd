...
#define P2P_GO_INTENT_MAX 15

/*
 * Check if the driver supports STA+P2P concurrency by querying
 * NL80211_ATTR_INTERFACE_COMBINATIONS from the wiphy info.
 * Returns true if the wiphy supports running both a STA and
 * P2P_CLIENT or P2P_GO interface simultaneously.
 */
static bool p2p_check_concurrency(struct p2p_device *dev)
{
	/*
	 * TODO: Parse NL80211_ATTR_INTERFACE_COMBINATIONS from wiphy
	 * for combinations containing both NL80211_IFTYPE_STATION and
	 * NL80211_IFTYPE_P2P_CLIENT / NL80211_IFTYPE_P2P_GO.
	 * For now return true assuming the driver supports concurrency
	 * if it created a P2P device at all.
	 */
	if (!dev || !dev->wiphy)
		return false;
...
/*
 * Resolve GO intent negotiation.
 * Per Wi-Fi P2P Specification v1.7 Section 3.1.4.2:
 * - Compare intent values (0-15)
 * - Higher intent wins GO role
 * - If tied, use tie-breaker bit
 * Returns true if we should be GO.
 */
static bool p2p_resolve_go_intent(uint8_t own_intent,
				bool own_tie_breaker,
				uint8_t peer_intent,
				bool peer_tie_breaker)
{
	...
	/* Intent values equal, use tie-breaker */
	return own_tie_breaker;
}
...
static void p2p_group_start(struct p2p_device *dev)
{
	...
	/* Check if concurrency is supported for STA+P2P GO */
	if (p2p_check_concurrency(dev))
		l_info("P2P GO: STA+P2P concurrency supported");
 ...
 * Exposed on /p2p/groupN paths when acting as GO
 */
static struct l_dbus_message *p2p_group_disconnect(struct l_dbus *dbus,
					struct l_dbus_message *message,
					void *user_data)
{
	struct p2p_device *dev = user_data;
...
static bool p2p_group_property_get_frequency(struct l_dbus *dbus,
				struct l_dbus_message *message,
				struct l_dbus_message_builder *builder,
				void *user_data)
{
...
static bool p2p_group_property_get_persistent(struct l_dbus *dbus,
				struct l_dbus_message *message,
				struct l_dbus_message_builder *builder,
				void *user_data)
{
	...
	l_dbus_message_builder_append_basic(builder, 'b', &persistent);

	return true;
}

static struct l_dbus_message *p2p_group_property_set_persistent(
				struct l_dbus *dbus,
				struct l_dbus_message *message,
				struct l_dbus_message_builder *builder,
				void *user_data)
{
	/*
	 * TODO: When set to true, save group credentials via
	 * p2p_persistent_group_save() for later reinvocation.
	 */
	return dbus_error_not_supported(message);
}

static void p2p_group_interface_setup(struct l_dbus_interface *interface)
{
	l_dbus_interface_method(interface, "Disconnect", 0,
				p2p_group_disconnect, "", "");

	l_dbus_interface_property(interface, "Frequency", 0, "u",
				p2p_group_property_get_frequency, NULL);
	l_dbus_interface_property(interface, "Persistent", 0, "b",
				p2p_group_property_get_persistent,
				p2p_group_property_set_persistent);
}
...
static int p2p_init(void)
{
	...
	/* Register the P2P Group interface for GO mode */
	if (!l_dbus_register_interface(dbus, IWD_P2P_GROUP_INTERFACE,
					p2p_group_interface_setup,
					NULL, false))
		l_error("Unable to register the %s interface",
			IWD_P2P_GROUP_INTERFACE);

	/* Initialize WFD Display session management */
	p2p_display_init();
...
static void p2p_exit(void)
{
	...
	p2p_display_exit();
	l_dbus_unregister_interface(dbus, IWD_P2P_GROUP_INTERFACE);
